...
> v0 由 `CLARIFICATION.md` 转换而来，只做模块划分。v1 锁定了运行环境约束、首个研究方向 profile 与若干悬空的架构决定。v1.1 加入了讨论/验证双模式（0.2）与网络分流。v1.2 在 §5 定下 Phase 1 的 State schema、Briefing/Handoff 契约、Task 契约与增量提交协议；v1.3 在 §5.5–5.8 定下 Phase 2 的 Paper 与证据出处、评判类任务、模式与交棒、Paper Request Queue 四个契约；v1.4 在 §5.9–5.14 定下 Phase 2.5（自演进）的契约（2026-09-23 用户确认并修改）。其余模块仍不涉及 API 签名和代码结构。
...
0. **模式门（Mode Gate）** —— 0.2 的落实处，优先于以下一切逻辑
   ...
   - **夜间预习（已定保留）**：讨论模式下若窗口空闲，agent 去做文献查证，结果在你下次打开前端时已就位。仅限文献，不含实验与方向变更。
     可关（2026-09-25 用户要求）：`AR_PREP_AUTO=0` 时不再开始预习（进行中的就地结束），Chat 页不显示“Tonight, look into”；空闲自动演进（§5.18.2）照常。
...
19. **理解的星标与手动合并**：理解可标星（星标的以全文进 briefing，其余一行）；研究者可勾选几条理解自己写合并后的表述（§5.17）。
...
以下 5.1–5.4 为 Phase 1 定下的具体形态（v1.2）；5.5–5.8 为 Phase 2 的四个契约（v1.3，2026-09-23 用户确认：摘要级证据规则、无人值守额度默认值、H002 保持 disputed 单列 unknown、已有对象的署名线索由人审过 diff 后中性化）；5.9–5.14 为 Phase 2.5（自演进）的契约（v1.4，2026-09-23 用户确认：Phase 2 judge 不加 --restricted、推演看得到 Insight、角度不固定、prior_work 不淘汰、自演进内容全部留存可送验证或记成理解）。机器可读的单一定义在 `autoresearch/schema.py`，本节是它的说明；二者冲突时以改本节为准、再改代码。
...
> **v1.9 修改（2026-09-25 用户决定，见 §5.19）**：Tidy up 只做理解的**聚合**，不再提 resolve 与改写候选；取消“>12 / >8 才提示”的门槛；夜间空闲时自动跑。下文保留原文作记录。
...
### 5.17 理解的可读性、星标与手动合并；自演进以新理解为出发点（Phase 2.8，v1.7，2026-09-24 用户提出）

**动机**。真实使用中发现：(1) 理解直接从讨论上下文里蒸出来，写法依赖讨论里的指代与临时叫法，过后再读“一下子 get 不到”；
(2) 理解仍然偏多，且每条都以全文进每场讨论；(3) 合并只能由蒸馏或 Tidy up 提议，研究者手里没有入口；
(4) 研究者希望新形成的理解能被系统自动“往前推”——经讨论确定为：**自演进拿它们当出发点**，而**不做**对理解的自动审查
（评判类任务看不到理解的原则不破例，§5.6）。

#### 5.17.1 理解要能脱离讨论读懂

- 蒸馏与讨论协议的 insight 规则追加**独立表述**要求：先一句结论，再用两三句交代它针对什么问题、在什么设定下成立；
  讨论里临时起的叫法、缩写要展开；不用“上面那个方案”“这种做法”一类指代；按“读者没看过这场讨论”来写。
- **已有理解的改写**（v1.9 起取消，§5.19：Tidy up 只做聚合）：Tidy up（§5.16.3）增加第三种候选——针对**读不懂的** active 理解提 `kind: revision`（target=IN###），
  只改措辞、**不改观点、不改牢固程度**；确认时走既有的 `insights.revise`（新 IN 取代旧的）。这是 revision 候选“只出自讨论”的唯一例外，
  source 为整理任务号。整理任务仍不产新想法。

#### 5.17.2 星标（理解的活跃集）

- 理解可选 `starred: true`，只由人在前端切换（每次一次 `ar-human` 提交），agent 不能改。语义同问题的活跃集（§5.16.2）。
- **briefing 第 3b 章分层**（discuss / distill）：星标的 active 理解给全文；其余 active 理解每条一行（id、牢固程度、陈述开头截到约 140 字），并写明全文在 `insights/`、需要时自行 Read。
  聚焦讨论的聚焦对象照旧在 2b 给全文。Tidy up 的第 10 章仍给全部 active 理解全文（它的工作就是看全）。自演进基本盘仍给全部 active 理解全文（推演任务读不到文件）。
- 前端：Research 页“Understanding”里星标的排最前，行首 ☆ / ★ 直接切换；对象页有 Star / Unstar。升级后首次提示“把最重要的理解标上星”（同 §5.16.5 的提示机制）。
- 星标不设上限；超过 5 条时提示“星标太多等于没有星标”。

#### 5.17.3 手动合并

- Research 页“Understanding”加 **Merge…**：勾选 ≥2 条 active 理解，研究者**自己**写合并后的表述、选 firmness（可选写“什么会让我改观”与合并说明），确认即合并。
  不经候选、不花额度；对象页也有 “Merge with…”（预选当前这条）。
...
- 首版基本盘的“当前理解”一章把两类理解标为**本次重点**：研究者星标的，以及**上一次自演进之后新增的**
  （不在上一个 session 首版基本盘里的 active 理解；没有上一个 session 时只看星标）。重点条目在该章最前，带“【本次重点】”标记与一句说明。
- 推演协议：若基本盘有本次重点，本 session 的推演链**优先**以它们为切入点——在上面更进一步、找出它们推不出的东西、或挑战它们；
  其他切入点仍然允许（角度不固定的原则不变，§5.12），但要在 checkpoint 里说明为什么不从重点出发。
- 基本盘仍是冻结快照：session 中途新增或星标的理解，下一次进入时才成为重点。
...
### 5.19 Tidy up 只做聚合，夜间自动跑（v1.9，2026-09-25 用户决定，修改 §5.16.3 与 §5.17.1）

**起因**。真实使用中 Tidy up 一共提了 3 条候选，全是“改写成独立表述”（每条确认后生成一条新 IN 取代旧的），一次合并都没有——
研究者看到的是“Tidy up 还会蹦出新的 insight”，而想要的是把越来越多的理解**聚合**起来。另外“active 理解 > 12 才提示”的门槛不合适：理解少的时候也需要聚合。

**规则**：

1. **只做聚合**：整理任务只提一种候选——`kind: insight` 带 `supersedes`（≥2 条 active 理解合成一条）。不再提 resolve（结问题照旧由讨论蒸馏提出、或研究者在问题页手动结），
   不再提改写（§5.17.1 的改写取消；读不懂的理解由研究者手动 Revise）。MCP 层与 `candidates.propose(tidy=True)` 双重限制。
2. **怎么聚**：把讲同一件事、或互相补成一个更完整看法的理解归成一组，每组合成一条；合成的表述保留各条真正的内容与分歧、独立可读；一条理解最多进一组；
   有实质冲突的不合；已被研究者丢弃过的合并（候选区“已丢弃”附理由）不换个说法再提。briefing 第 10 章只给全部 active 理解的全文（不再给问题）。可以交白卷。
3. **没有门槛**：只要有 ≥2 条 active 理解，Tidy up 按钮就在，Understanding 一栏的标题处随时可点；删除“>12 / >8”提示。
4. **夜间自动**（`AR_TIDY_AUTO`，缺省开）：讨论模式下研究者离开后的空闲窗口（同 §5.18.2 的条件与无人值守份额），排在夜间预习与自动演进之后，每个空闲期至多一次；
   另外两个条件避免重复劳动：自上一次整理以来 active 理解的集合**有变化**，且上一次整理提出的合并候选都已处理（没有仍 pending 的）。
5. 产出只进 Inbox，全部由研究者批阅（合并对话框里可改措辞、选牢固程度）。
