...
0. **模式门（Mode Gate）** —— 0.2 的落实处，优先于以下一切逻辑
   ...
   - **夜间预习**：讨论模式下若窗口空闲，允许 agent 把本次讨论涉及的 assumption 拿去做文献查证，结果在你下次打开前端时已就位。仅限文献，不含实验与方向变更。
...
7. **停止条件是结构性的，不是自我评价的**：
   ...
   “我觉得这个想法不错”**不是**停止条件——自评新颖性恰好是模型最不可靠的能力。这三条同时也是防止它写散文的闸门：要求给出证伪条件，就逼它把直觉收敛成一个有内容的断言。
8. **事后对抗性接地**：交给 M4 的 Grounding Pass，由**另一个开着检索的任务**执行，只标注不改写。
9. **选择与排序**：多条推演产出后做一次选择，只把最值得你花十分钟的送进候选区。**允许且应当输出“本轮没有值得你看的东西”**。
10. **产出形态**：`Idea` 对象（见 M1）。要求直觉层面的洞察，不要求工程细节。
11. **中断恢复**：推演链增量提交，5h 截断后从断点续推。半条链是合法的可恢复状态。
