...
1. **H003 的署名线索中性化**（你选了“我出 diff、你确认”）：diff 在 `spike/phase2/H003-neutralize.diff`。只改 `validation` 的措辞、把“## 来由”换成“（来由见候选 C001。）”——原话在 C001 里一字不少。你确认后我以 `ar-human` 身份提交到 `~/autoresearch/state`。真实 State 里只有 H003 有线索。
2. **前端需要重启 `./ar serve` 才生效**（后端改动很多）。你的 tmux 里那个进程我没动。
