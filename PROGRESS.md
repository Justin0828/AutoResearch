...
## Phase 2.7 设计（2026-09-24，分支 `phase2.7`，待实现）

- 用户真实使用一轮后（DS001 → Q001–Q005、IN001）觉得“越垒越多”。原因是结构性的：问题接受后没有“被回答”的出口，每个开着的问题都以全文进每场讨论，理解只增不减。
...
### 试用中发现并修掉的（2026-09-24）

- **候选引用候选**：重启后第一次蒸馏（DS002，C008–C016）把同批候选 C### 填进了 insight 候选的 relates_to / informs；提交时只查“存在”，
  确认后被原样抄到 IN002–IN004（修订又把 informs 继承到 IN005），校验报 8 个错误。漏检从 Phase 2.6 起就在，很可能是 2.7 协议里
...
- **夜间预习可关**：`AR_PREP_AUTO=0`（不影响空闲自动演进）。已合入 main。
- **对话框不再丢输入**：用户手写的 insight 因缺根基被拒，对话框随即关闭、内容丢失且无法找回（服务端不记被拒请求）。`ask()` 改为提交前检查（check）、
  在对话框内提交（submit，失败保持打开并显示错误）、按标题在本地存草稿（成功才清，取消 / 失败 / 刷新后自动填回，可丢弃），全部带文字的对话框都改了。
- **Tidy up 只做聚合，夜间自动**（DESIGN §5.19，v1.9）：真实使用中 Tidy up 提的 3 条候选全是“改写成独立表述”（确认后生成新 IN），一次合并都没有，
  用户感受是“蹦出新的 insight”。改为只提合并候选（MCP 层与 candidates 双重限制，不再提 resolve / 改写）；briefing 第 10 章只给理解；取消 >12 / >8 的提示门槛，
  按钮移到 Understanding 一栏、≥2 条理解即可点；讨论模式下空闲时自动跑一次（排在预习与自动演进之后；理解集合有变化且上次的合并都已处理才跑；`AR_TIDY_AUTO`）。
  测试 149 个全过；真实 State 副本上手动 Tidy up 提出 1 条合并候选。
