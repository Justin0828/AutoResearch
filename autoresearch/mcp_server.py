...
from autoresearch import (candidates, config, discussion, library, modes, papers,  # noqa: E402
                          reviews, schema)
...
TOOLS = [
    ...
    ("annotate_grounding", t_annotate_grounding,
     "对抗性接地的结论：这个对象（假设 / 前提 / 候选）有人做过吗、有没有直接反驳。只标注，不改被核查的对象。",
    ...
    ("propose_candidate", t_propose_candidate,
     "把讨论中出现的研究内容提交到候选区，等人确认。讨论把一个**已有**对象说得更准了（大方向不变）时，"
     "用 kind=revision 提修订（target + base_revision，statement 写修订后的完整表述），不要新建近义对象。"
     "新建对象按**当前角色**分类："
