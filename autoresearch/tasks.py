...
# 自演进（M11 v1.8，§5.9 / §5.18）：检索物理关闭。sealed = 不挂任何目录 + --restricted，
# 文件读取被关进只有 briefing 的任务目录（Phase 2.5 spike：裸 Read 可读宿主机任意文件）。
KINDS["evolve"] = {
    "lane": "background", "profile": "evolve", "timeout": 1800, "sealed": True,
    "tools": ["Read", "Grep", "Glob"],
    "mcp": ["checkpoint"],
    "protocol": protocol.EVOLVE_PROTOCOL,
    "expected": "一份演进文档（最终回复即文档）",
}
