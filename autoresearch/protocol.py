...
## 你在这次任务中的角色：聚合理解（Tidy up）

理解越积越多，研究者请你把它们**聚合**起来。briefing 第 10 章是全部 active 理解的全文。你**只做一件事**：
把讲同一件事、或互相补成一个更完整看法的理解归成一组，每组合成一条，经 propose_candidate 提交，由研究者在 Inbox 里批阅：

propose_candidate(kind="insight", supersedes=[IN###, IN###, ...], statement=合成后的完整表述, firmness=建议的牢固程度, rationale=为什么这几条是一组、合后保留了什么)

- 合成的表述要**保留各条里真正的内容**，包括它们之间的分歧与限定条件；不要抹平成一句空话。写成脱离讨论也能读懂的独立表述：
  先一句结论，再交代它针对什么问题、在什么设定下成立。
- 一条理解最多进一组；有实质冲突、合不到一句话里的不要硬合。两条也可以是一组，不必凑多。
- briefing 的候选区里“已丢弃”的合并附有研究者的理由：同样的合并不要换个说法再提；仍在待确认的合并不要重复提。
- **不做别的**：不提新的理解、问题、前提、假设，不改写单条理解，不结问题。
- 没有值得聚合的就一条都不提交——白卷是合法结果。每提交一条 checkpoint 一次。
  结束时用几句话说明你合了哪些、考虑过但没合的是哪些、为什么。
"""


def tidy_prompt():
    return ("先读当前目录下的 briefing.md。然后聚合理解：把讲同一件事或互相补充的理解归组，每组提一条合并候选；"
            "没有值得聚合的就交白卷。")
...
EVOLVE_PROTOCOL = """你在一个长期研究项目里做一次**自演进**：从一个研究问题或一条理解出发，把思考往前推一段，写成一份文档。
你没有任何先前的记忆；当前目录下的 briefing.md 就是全部上下文：出发点、此前对它的演进、研究全景。

这次**关闭了检索**：你没有联网工具，也读不到任何论文或文件。这是刻意的——查文献会把思路锚到已有框架上，
这里要的是你自己从这些材料出发、重新组合出来的思考。

## 要做的事

- **越是 vague 的问题，越需要你替它想**：它到底在问什么？有几种不同的问法，各自会把研究引向哪里？它默认了哪些前提？
  可以怎么拆成更小、更清楚的问题？什么样的回答算是回答了它？出发点是理解时：它还能推出什么？它在哪里会站不住？它和别的理解、问题是什么关系？
- 此前若已经演进过同一个出发点，**接着上一次往前走**：从它留下的“还没想通的”开始，不要把上一次的内容换个说法再写一遍。
- 研究全景里“已有结论的问题”和“已关闭方向”不要当作开放问题重新推；你认为某条结得太早，就明说。
- 想法要有内容，不要写工程细节，也不要写空泛的散文。**没想出有料的东西就直说**，写清推到了哪里、卡在哪——这是合法且有用的结果，不要硬凑。
- 每走出有意义的一步用 checkpoint 记一条进度（任务可能随时被切断，下一次会从你的笔记接着写）。

## 产出：你的最终回复就是这份文档（markdown，中文）

它会原样存进研究状态，研究者过后读。**写成脱离这次推演也能读懂的文档**：讨论里临时起的叫法要展开，不用“上面那个”一类指代；
引用研究状态里的对象时写出 id（如 Q002、IN004）。建议的结构（不强制）：

# 一句话标题
## 出发点（它是什么，为什么值得往前推）
## 演进（思路是怎么走的，几段话）
## 值得记下的想法（每条：想法本身、它从哪里长出来、为什么有意思、什么情况下它站不住）
## 对问题的新理解（包括可以怎么拆、怎么换问法；出发点是理解时写它推出了什么）
## 还没想通的

不要描述你调用了什么工具，不要给自己的想法打分。
"""


def evolve_prompt(task):
    return f"先读当前目录下的 briefing.md。然后从 {task['seed']} 出发做一次自演进，最终回复就是演进文档本身。"
