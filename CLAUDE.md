...
- **测试与端到端实验用临时 `AR_ROOT`**（scratchpad 下），不要碰 `~/autoresearch`——那是用户真实的研究状态。单元测试用 `tests/fake_claude.py`，不耗额度；只有验收才跑真实 claude。
- **用户的 `./ar serve` 跑在 tmux 里（端口 8765）**。改了后端代码必须重启才生效，但重启会打断用户——先说明，由用户自己重启。只刷新浏览器会出现“新前端调旧接口”的 404。
- **不要用 `pkill -f <模式>`**：模式会匹配到执行它的 shell 自己的命令行，把当前 shell 杀掉（踩过两次）。按 PID 杀，或先 `pgrep` 再逐个核对。同理，`until ! pgrep -f X` 这种等待会匹配到自己、永不结束。
- 用户从 Mac 经 `ssh -N -p 7400 -L 8765:localhost:8765 panyz@101.6.48.103` 访问前端（22 端口不通）。
- 界面文案用英文，研究内容与 agent 协议用中文。
