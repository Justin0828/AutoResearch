...
10. **自演进（v1.8 重做）**：从一个问题或理解出发，在**检索关闭**的条件下把思考往前推一段，写成一份**演进文档**（EV###）。
    越 vague 的问题越优先。问题 / 理解页点「Evolve ✦」，或在 **Evolution** 页点「Evolve…」让系统挑；你离开 90 分钟后
    （夜间预习之后）系统也会自己挑一个跑一篇，每个空闲期至多一篇（`AR_EVOLVE_AUTO=0` 关掉）。同一出发点再演进时会接着上一篇往下写。
    文档不改任何研究对象：读完有用的自己记成理解，或在文档页「Discuss this」开讨论，在讨论里蒸馏。
11. **Evolution 页**：全部演进文档（最新在前，从哪出发、是你点的还是空闲时自动的），以及在排队或在跑的演进任务。
12. **让研究收敛（Phase 2.7）**：Research 页以**问题树**为主视图（根是主问题；没挂母问题的在「Not placed yet」，
    ...
    （结问题的候选由讨论蒸馏提出；Tidy up 从 v1.9 起只做理解的聚合，见下一条。）
13. **理解的星标与合并（Phase 2.8）**：Understanding 行首 ☆ → ★ 标星，只有星标的理解以全文进讨论的 briefing，其余一行；
    「Merge…」勾两条以上、自己写合并后的表述即合并。以问题为出发点演进时，星标的与上次演进之后新形成的理解会一并带上。
    **Tidy up**（Understanding 一栏标题处，≥2 条理解就能点）：AI 读全部理解，把讲同一件事或互相补充的归组，每组提一条**合并候选**进 Inbox，
    你在确认对话框里改措辞、选牢固程度；它不提新理解、不改写单条、不结问题，没有可合并的就交白卷。你离开后的空闲窗口也会自动跑一次
    （理解有变化、且上次的合并都处理完才跑；`AR_TIDY_AUTO=0` 关掉）。
...
| `AR_PREP_IDLE_MIN` | `90` | 你离开多少分钟后开始夜间预习（空闲自动演进也用这个时长） |
| `AR_PREP_AUTO` | `1` | 夜间预习（离开后自动读论文）；`0` 关掉，不影响空闲自动演进 |
| `AR_PREP_MAX_TASKS` | `6` | 每次夜间预习最多几个任务 |
| `AR_EVOLVE_AUTO` | `1` | 你离开后的空闲窗口里自动演进一篇；`0` 关掉（手动仍可用） |
| `AR_TIDY_AUTO` | `1` | 你离开后的空闲窗口里自动聚合理解一次（只提合并候选进 Inbox）；`0` 关掉（手动仍可用） |
...
| EV | Evolution | 演进文档：从一个问题或理解出发、检索关闭时演进的一段思考。只是文档，不改任何对象 |
...
- **演进看不到检索**：演进任务只给 Read/Grep/Glob，不挂 State 与论文库，加 `--restricted` 把读取关进只有 briefing 的任务目录，
  MCP 只注册 checkpoint；开跑前自检，没封死就不启动（实测见 `spike/phase2.5/README.md`）。
...
autoresearch/          代码
  ...
  evolution.py         自演进：选题（越 vague 越优先）、检索关闭的 briefing、把回复写成演进文档（§5.18）
